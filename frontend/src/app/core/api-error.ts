...
/**
 * Turns an API error into human-readable, field-aware messages. Parses RFC 7807 ProblemDetails
 * (`errors` per field + `detail`). Infrastructure failures (status 0 / 5xx / 429) get an honest
 * message — never "check your input". Server-sent validation text comes through verbatim.
 */
...
/** Field-keyed validation errors from a ProblemDetails 400 (key → messages). */
export function parseFieldErrors(error: unknown): Record<string, string[]> {
  if (error instanceof HttpErrorResponse && error.error?.errors && typeof error.error.errors === 'object')
